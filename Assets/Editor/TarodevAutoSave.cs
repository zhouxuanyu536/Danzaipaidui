...
namespace Tarodev
{
    ...
    /// </summary>
    [InitializeOnLoad]
    public static class AutoSaveListener
    {
        static AutoSaveListener()
        {
            //Undo.postprocessModifications += OnUndo;
        }

        private static UndoPropertyModification[] OnUndo(UndoPropertyModification[] modifications)
        {
            if (modifications.Length > 0)
            {
                TarodevAutoSave.SaveScenes(); // 检测到修改，立即保存
            }
    ...
    public class TarodevAutoSave : Editor
    {
        ...
        private static async Task SaveInterval(CancellationToken token)
        {
            ...
            {
                await Task.Delay(_config.Frequency * 1000 * 30, token);
